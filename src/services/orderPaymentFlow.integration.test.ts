  ...
  const supabase = {
    ...
    async rpc(functionName: string, params: Record<string, any>) {
      ...
      if (functionName === 'verify_client_po_and_confirm_order') {
        ...
        if (order.status !== 'PENDING_PO') {
          return { data: null, error: { message: 'Order must be in PENDING_PO status for verification' } };
        }
...
describe('RFQ -> Quote -> Order -> Payment confirmation integration', () => {
  ...
  it('moves verified client PO orders from PENDING_PO to PENDING_PAYMENT', async () => {
  ...
  it('blocks invalid status transition when confirming payment', async () => {
    ...
    ).rejects.toThrow('Invalid order status transition: PENDING_PO -> PAYMENT_CONFIRMED');
  });
